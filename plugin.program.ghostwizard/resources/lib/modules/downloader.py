...
class Downloader:
	...
	def download_build(self, name,zippath,meth='session', stream=True):
		...
		dp.create(name + ' - ' + str(length2) + ' MB', 'Downloading uw build...')
		dp.update(0, 'Downloading uw build...')
		cancelled = False
		...
		if length:
			...
			if meth in ['session', 'requests']:
				for chunk in response.iter_content(chunk_size=max(1000000,500000)):
					...
					dp.update(perc, 'Downloading uw build...' + '\n' + str(size2) + '/' + str(length2) + 'MB')
					if dp.iscanceled():
						cancelled = True
						break
			elif meth in 'urllib':
			...
			dp.update(50, 'Downloading uw build...')
		...
		if cancelled:
			...
			dialog.ok('Gestopt', 'Download gestopt')
			return
		if length:
