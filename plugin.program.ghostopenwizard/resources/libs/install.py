...
def wipe():
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Berekend bestanden en mappen".format(CONFIG.COLOR2) + '\n' + '\n' + 'Geduld![/COLOR]')
    total_files = sum([len(files) for r, d, files in os.walk(xbmcPath)])
    del_file = 0
    progress_dialog.update(0, "[COLOR {0}]Verzameld uw voorkeuren die bewaard moeten blijven.[/COLOR]".format(CONFIG.COLOR2))
    ...
    progress_dialog.update(0, "[COLOR {0}]Ruimt bestanden en mappen op:".format(CONFIG.COLOR2))
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        ...
        for name in files:
            ...
            if name == 'sources.xml' and fold[-1] == 'userdata' and CONFIG.KEEPSOURCES == 'true':
                ...
                progress_dialog.update(int(tools.percentage(del_file, total_files)), '\n' + '[COLOR {0}]Bestand: [/COLOR][COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name))
                try:
                    os.remove(os.path.join(root, name))
                except Exception as e:
        ...
        if progress_dialog.iscanceled():
            ...
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Schone installatie gestopt[/COLOR]".format(CONFIG.COLOR2))
            return False
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        dirs[:] = [d for d in dirs if d not in exclude_dirs]
        for name in dirs:
            progress_dialog.update(100, '\n' + 'Ruimt lege map op: [COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name))
...
def fresh_start(install=None, over=False):
    ...
    if over:
        ...
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE,
                                       "[COLOR {0}]Uw huidige Kodi instellingen".format(CONFIG.COLOR2),
                                       "naar standaard terugzetten",
                                       "voordat de lokale backup wordt geinstalleerd?[/COLOR]",
                                       nolabel='[B][COLOR red]Nee[/COLOR][/B]',
                                       yeslabel='[B][COLOR green]Ga door[/COLOR][/B]')
    elif install:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Uw huidige Kodi instellingen".format(CONFIG.COLOR2),
                                       "naar standaard terugzetten",
                                       "v [COLOR {0}]{1}[oordat u gaat installeren/COLOR]?".format(CONFIG.COLOR1, install),
                                       nolabel='[B][COLOR red]Nee[/COLOR][/B]',
                                       yeslabel='[B][COLOR green]Ga door[/COLOR][/B]')
    else:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Uw huidige Kodi instellingen".format(CONFIG.COLOR2) +' \n' + "naar standaard terugzetten?[/COLOR]", nolabel='[B][COLOR red]Nee[/COLOR][/B]', yeslabel='[B][COLOR green]Ga door[/COLOR][/B]')
    if yes_pressed:
        wipe()
        ...
        if over:
            ...
            dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Om alle wijzigingen door te voeren dient u Kodi af te sluiten, klik nu op OK om Kodi af te sluiten[/COLOR]".format(CONFIG.COLOR2))
            from resources.libs import update
...
def install_apk(name, url):
    ...
    if tools.platform() == 'android':
        ...
        if os.path.exists(lib):
            redownload = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] Bestaat teeds. Opnieuw downloaden?'.format(CONFIG.COLOR1, apk),
                               yeslabel="[B]Redownload[/B]",
                               nolabel="[B]Installeer[/B]")
            yes = False
        else:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Downloaden en installeren: ".format(CONFIG.COLOR2),
                                   "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                                   yeslabel="[B][COLOR green]Downloaden[/COLOR][/B]",
                                   nolabel="[B][COLOR red]Stop[/COLOR][/B]")
                                   
            if not yes:
                logging.log_notify(CONFIG.ADDONTITLE,
                               '[COLOR {0}]FOUT: Installatie gestopt[/COLOR]'.format(CONFIG.COLOR2))
                return
        ...
        if yes or redownload:
            ...
            if not response:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]APK installatie: ongeldige Apk Url![/COLOR]'.format(CONFIG.COLOR2))
                return
        ...
        dialog.ok(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] downloaden naar[COLOR {}]{}[/COLOR]. Als installatie niet start, ga dan naar de juiste locatie.'.format(CONFIG.COLOR1, apk, CONFIG.COLOR1, path))
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           '[COLOR {0}]FOUT: Geen Android apparaat[/COLOR]'.format(CONFIG.COLOR2))
