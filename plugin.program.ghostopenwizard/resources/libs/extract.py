...
def all_with_progress(_in, _out, dp, ignore, title):
    ...
    try:
        ...
        errors += 1
        error += '%s\n' % e
        logging.log('Fout bij controlle zip: {0}'.format(str(e)), level=xbmc.LOGERROR)
    ...
    for item in zin.infolist():
        ...
        try:
            ...
            logging.log("[ASCII Check] Onjuist karakter gevonden in bestand: {0}".format(item.filename))
            continue
        ...
        line1 = '{0} [COLOR {1}][B][Fouten:{2}][/B][/COLOR]'.format(title,
                                                                    CONFIG.COLOR2,
                                                                    errors)
        line2 = '[COLOR {0}][B]Bestand:[/B][/COLOR] [COLOR {1}]{2}/{3}[/COLOR] '.format(CONFIG.COLOR2,
                                                                                     CONFIG.COLOR1,
                                                                                     count,
                                                                                     int(nFiles))
        line2 += '[COLOR {0}][B]Afmeting:[/B][/COLOR] [COLOR {1}]{2}/{3}[/COLOR]'.format(CONFIG.COLOR2,
                                                                                     CONFIG.COLOR1,
        ...
        if skip:
            ...
            try:
                ...
                errormsg = "[COLOR {0}]Bestand:[/COLOR] [COLOR {1}]{2}[/COLOR]\n".format(CONFIG.COLOR2,
                                                                                      CONFIG.COLOR1,
                                                                                      file[-1])
                errormsg += "[COLOR {0}]Map:[/COLOR] [COLOR {1}]{2}[/COLOR]\n".format(CONFIG.COLOR2,
                                                                                         CONFIG.COLOR1,
                                                                                         item.filename.replace(file[-1], ''))
                errormsg += "[COLOR {0}]Fout:[/COLOR] [COLOR {1}]{2}[/COLOR]\n\n".format(CONFIG.COLOR2,
                                                                                          CONFIG.COLOR1,
                                                                                          str(e).replace('\\\\', '\\')
                                                                                          .replace("'{0}'"
    ...
    if dp.iscanceled():
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]uitpakken gestopt[/COLOR]".format(CONFIG.COLOR2))
        sys.exit()
