...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = "Er is nog geen build op uw systeem van {0}.\n\nSelecteer 'Build Menu' om een build te installeren of 'Ignore' om deze boodschap niet meer te zien.\n\nBedankt voor het kiezen van onze {1}.".format(CONFIG.ADDONTITLE, CONFIG.ADDONTITLE)
...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fanart = kwargs['fanart']
            self.msgupdate = "Update beschikbaar voor huidige build:\n[COLOR {0}]{1}[/COLOR]\n\nHuidige versie: v[COLOR {2}]{3}[/COLOR]\nLaatste versie: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Aanbevolen: Schone installatie[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
            self.msgcurrent = "U heeft de laatste versie van build:\n[COLOR {0}]{1}[/COLOR]\n\nHuidige versie: v[COLOR {2}]{3}[/COLOR]\nLaatste versie: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Aanbevolen: Schone installatie[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
    ...
    # update.doModal()
    # del update
    msgcurrent = 'U werkt met laatste versie van build: '
    msgupdate = 'Update beschikbaar voor huidige build: '
    build_name = '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name)
    current_version = 'Huidie versie: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, current)
    latest_version = 'Laatste versie: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
