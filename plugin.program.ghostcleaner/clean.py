# ============================================================
import xbmc,xbmcgui,xbmcvfs,xbmcplugin,xbmcaddon,os,shutil,sqlite3,json,urllib.request,urllib.error,urllib.parse,gzip,urllib.request,urllib.parse,urllib.error,sys,codecs,pickle
from distutils.util import strtobool
from xml.dom import minidom
addon_id = 'plugin.program.ghostcleaner'
            ...
            if jj["file"][:9] != "addons://" and jj["file"][:6] != "rss://" and jj["file"][:6] != "ftp://" \
                ...
                and jj["file"][:10] != "musicdb://" and jj["file"][:7] != "cdda://":
                    ...
                    if xbmcvfs.exists(afg):
                        xbmc.log("GhostClean >> SOURCE PATHS (" + k +") >> " + afg + " >> ok")
                    else:
                        xbmc.log("GhostClean >> SOURCE PATHS (" + k +") >> " + afg + " >> error")
...
def CalcDeleted():
    ...
    for i, line in enumerate(msizes):
        xbmc.log("GhostClean >> CALCULATED SAVINGS >> " + str(line))
                ...
                if get_filename(f) == "settings" or root == userDataPath:
                    ...
                    if os.path.isfile(pout):
                        try:
                            ...
                            xbmc.log("GhostClean >> COULDN'T DELETE OLD _NEW FILE")
                    ...
                    for line in fp:
                        if userDataPath in line:
                            ...
                            xbmc.log("GhostClean >> COMPACTING PATH: " + root + "\\" + f)
                            wasChanged = True
                        else:
                            newline = line
                    ...
                    if wasChanged:
                        ...
                        try:
                            ...
                            xbmc.log("GhostClean >> COULDN'T DELETE OR RENAME ORIGINAL FILE")
                    else:
                        try:
                            os.remove(pout)
                        except Exception:
                            xbmc.log("GhostClean >> COULDN'T DELETE NEW FILE")
...
def CleanTextures(iMode):
    ...
    try:
        ...
        xbmc.log("GhostClean >> SQL ERROR IN Textures13: " + str(e))
        data = None
    ...
    for d in data:
        ...
        if not os.path.isfile(thumbPath):
            try:
                ...
                if RockDebug:
                    xbmc.log("GhostClean >> DELETED RECORD FROM DB: " + str(thumbPath))

                counter += 1
            except Exception as e:
                xbmc.log("GhostClean >> SQL ERROR IN Textures13 DELETING ID: " + str(recID) + " >> " + str(e))
        ...
        if fileName.startswith("http://") or fileName.startswith("https://") or fileName.startswith("image://") or fileName.endswith("/transform?size=thumb"):
            ...
            if not os.path.isfile(fileName):
                try:
                    ...
                    if RockDebug:
                        xbmc.log("GhostClean >> DELETED RECORD FROM DB: " + str(fileName))
        ...
        if entry[3] in cleanIt and not entry[4]:
            ...
            if os.path.exists(clear_cache_path):
                            ...
                            if entry[3] == 'packages' and ignore_packages > 0:
                                if f in plist:
                                    try:
                                        ...
                                        TotalfileSize += fileSize
                                        if RockDebug:
                                            xbmc.log("GhostClean >> DELETED >>" + f)
                                    except Exception as e:
                                        xbmc.log("GhostClean >> CAN NOT DELETE FILE >>" + f + "<< ERROR: " + str(e))
                            ...
                            for d in dirs:
                                if os.path.join(root, d) != anigPath and os.path.join(root, d) != arccPath:
                                    try:
                                        ...
                                        if RockDebug:
                                            xbmc.log("GhostClean >> DELETED >>" + d)
                                    except Exception as e:
                                        xbmc.log("GhostClean >> CAN NOT DELETE FOLDER >>" + d + "<< ERROR: " + str(e))
                    else:
                        pass
                ...
                if TotalfileSize > 0:
                    ...
                    xbmc.log("GhostClean >> CLEANING >> " + mess)
                    intTot = TotalfileSize / 1048576.00000001
                    grandTotal += TotalfileSize
                else:
                    strEndMessage += (entry[3].title() + " [COLOR blue](" + entry[0] + ") :[/COLOR] ")
...
def CompactDatabases(iMode):
    ...
    if os.path.exists(dbPath):
        ...
        for f in files:
            ...
            xbmc.log("GhostClean >> COMPACTED DATABASE >>" + f)
...
def getLocalRepos():
    ...
    for f in repos:
        ...
        if RockDebug:
            xbmc.log("GhostClean >> INSTALLED REPOS >>" + f["addonid"])
...
def getLocalAddons():
    ...
    for f in addons:
        if f["type"] != "xbmc.addon.repository":
            ...
            if RockDebug:
                xbmc.log("GhostClean >> INSTALLED ADDONS >>" + f["addonid"])
    ...
    for d in addonData:
        ...
        if d not in installedAddons:
            ...
            try:
                ...
                xbmc.log("GhostClean >> DELETING UNUSED ADDON DATA FOLDER >>" + fullName)
                deleted += 1
            except Exception as e:
                xbmc.log("GhostClean >> ERROR DELETING UNUSED ADDON DATA FOLDER: " + str(e))
...
def getJson(method, param1, param2, retname):
    ...
    if 'result' in py and retname in py['result']:
        ...
        if RockDebug:
            xbmc.log("GhostClean >> READ SYSTEM SETTING >> " + method + ":" + param1 + "."+ param2 + " >> " + str(a) )
...
def ProcessAddons(iMode):
    ...
    for a in AddonsInstalled:
        ...
        if a in AddonsInRepo:
            xbmc.log("GhostClean >> ADDON >> " + a + " >> FOUND")
        else:
            xbmc.log("GhostClean >> ADDON >> " + a + " >> IS IN NO REPOSITORY")
...
def ProcessRepos(iMode):
    ...
    for r in repos:
        ...
        xbmc.log("GhostClean >> PROCESSING REPO >>" + r)
        ...
        if len(AddonsInRepo) == 0:
            xbmc.log("GhostClean >> REPO >> " + r + " >> " + repoxml + " >> EMPTY OR ERROR")
            ...
            if len(similar) == 0:
                xbmc.log("GhostClean >> REPO >> " + r + " >> CONTAINS NO LOCAL ADDONS")
                mess1 = __addon__.getLocalizedString(30121)
                strEndMessage += r + " [B][COLOR red]" + mess1 + "[/B][/COLOR]\n"
            else:
                if RockDebug:
                    for i in similar:
                        xbmc.log("GhostClean >> REPO >> " + r + " >> CONTAINS >>" + i)
...
def GetAddonsInRepo(netxml, repo):
    ...
    if os.path.exists(netxml):
        if RockDebug:
            xbmc.log("GhostClean >> LOCAL XML EXISTS >>" + netxml)

        repopath = getRepoPath(netxml)

        if not repopath and RockDebug:
            xbmc.log("GhostClean >> INFO TAG NOT FOUND IN REPO >>" + netxml)
        ...
        for ri in repopath:
            ...
            for addon in id:
                ...
                if RockDebug:
                    xbmc.log("GhostClean >> ADDON >>" + addon)

    else:
        if RockDebug:
            xbmc.log("GhostClean >> LOCAL XML DOES NOT EXIST >>" + netxml)
...
def getRepoPath(xmlFile):
    ...
    for r in infoTag:
        try:
            ...
            if RockDebug:
                xbmc.log("GhostClean >> REPO PATHS >>" + r.childNodes[0].nodeValue)
        except Exception as e:
            xbmc.log("GhostClean >> ERROR REPO PATHS >>" + str(e).encode('utf-8'))
...
#        f.close()
    ...
    try:
        ...
        xbmc.log("GhostClean >> ERROR PARSING REPO >> " + str(e).encode('utf-8'))
        return ""
    ...
    for r in infoTag:
        ...
        if addon != repo:
            ...
            if RockDebug:
                xbmc.log("GhostClean >> REPO-ADDON-LIST >>" + addon)
...
__addon__ = xbmcaddon.Addon(id='plugin.program.ghostcleaner')
...
xbmc.log("GhostClean >> STARTED VERSION %s" % (__version__))
...
xbmc.log('GhostClean SERVICE >> RUNNING APP...' + str(xbmcgui.Window(xbmcgui.getCurrentWindowId()).getProperty('GhostClean')))
