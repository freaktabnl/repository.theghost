...
def GetSetting():
    ...
    xbmc.log('GhostClean SERVICE >> SETTINGS CHANGED >> SERVICE RUN: ' + str(RockBackgroundRun))
    xbmc.log('GhostClean SERVICE >> SETTINGS CHANGED >> RUN EVERY DAYS: ' + str(lastrundays))
...
if __name__ == '__main__':
    xbmc.log("GhostClean SERVICE >> STARTED VERSION %s" % (__version__))
    ...
    if RockBackgroundRun:
        xbmc.log("GhostClean SERVICE >> SERVICE INIT >> LAST RUN " + str(time_difference_days) + " DAYS AGO, SET TO RUN EVERY " + str(lastrundays) + " DAYS, WITH DELAY OF " + str(autostart_delay) + " MINUTE(S)")
        ...
        if time_difference_days > lastrundays or lastrundays == 0:
            ...
            if __addon__.getSetting('lock') != 'true':
                ...
                xbmc.log('GhostClean SERVICE >> RUNNING AUTO...')
                AutoClean()
                __addon__.setSetting('auto_lastrun', str(int(round(time.time()))))
                __addon__.setSetting('lock', 'false')
    else:
        xbmc.log("GhostClean SERVICE >> SERVICE OFF")
    ...
    while True:
        if monitor.waitForAbort(2):
            xbmc.log('GhostClean SERVICE >> EXIT')
            break
        else:
            if RockBackgroundRun:
                iCounter += 1
                ...
                if iCounter > 1800:
                    ...
                    xbmc.log("GhostClean SERVICE >> LAST RUN " + str(time_difference_days) + " DAYS AGO, SET TO RUN EVERY " + str(lastrundays) + " DAYS (NOW: " + str(date_now) + ")")
                    ...
                    if time_difference_days > lastrundays:
                        if __addon__.getSetting('lock') != 'true':
                            ...
                            __addon__.setSetting('lock', 'false')
                            xbmc.log('GhostClean SERVICE >> END AUTO...')
